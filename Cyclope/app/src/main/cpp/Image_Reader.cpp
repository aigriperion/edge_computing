...
bool Image_Reader::DisplayImage(ANativeWindow_Buffer *buf, AImage *image) {
    ...
    if (presentRotation_ != 0) {
        ...
        switch (presentRotation_) {
            case 90:
                cv::rotate(rgbaScratch_, rgbaRotScratch_, cv::ROTATE_90_CLOCKWISE);
                oriented = &rgbaRotScratch_;
                break;
            case 180:
                cv::rotate(rgbaScratch_, rgbaRotScratch_, cv::ROTATE_180);
                oriented = &rgbaRotScratch_;
                break;
            case 270:
                cv::rotate(rgbaScratch_, rgbaRotScratch_, cv::ROTATE_90_COUNTERCLOCKWISE);
                oriented = &rgbaRotScratch_;
                break;
            default:
    ...
    // 3) Blit en plein écran (CENTER_CROP)
    if (!loggedOnce_) {
...
static inline void CenterCropResize(const cv::Mat& src, cv::Mat& dst) {
    ASSERT(src.data && dst.data, "CenterCropResize invalid mats");
    ASSERT(src.type() == CV_8UC4 && dst.type() == CV_8UC4, "CenterCropResize expects RGBA");

    const double scale = std::max(
            dst.cols / (double)src.cols,
            dst.rows / (double)src.rows
    );

    const int newW = (int) lround(src.cols * scale);
    const int newH = (int) lround(src.rows * scale);

    cv::Mat resized;
    cv::resize(src, resized, cv::Size(newW, newH), 0, 0, cv::INTER_LINEAR);

    const int x = std::max(0, (resized.cols - dst.cols) / 2);
    const int y = std::max(0, (resized.rows - dst.rows) / 2);

    cv::Rect roi(x, y, dst.cols, dst.rows);
    resized(roi).copyTo(dst);
}
...
void Image_Reader::ConvertYUV420ToRGBA(AImage *image, cv::Mat &outRGBA) {
    ...
    if (outRGBA.empty() || outRGBA.cols != srcW || outRGBA.rows != srcH) {
        outRGBA = cv::Mat(srcH, srcW, CV_8UC4);
        LOGI("[Image_Reader] RGBA scratch allocated: %dx%d", srcW, srcH);
    }
    ...
    for (int y = 0; y < srcH; y++) {
        ...
        const int uv_row_start = uvStride * ((y + srcRect.top) >> 1);
        const uint8_t *pU = uPixel + uv_row_start + (srcRect.left >> 1);
        const uint8_t *pV = vPixel + uv_row_start + (srcRect.left >> 1);

        uint32_t *dstRow = outRGBA.ptr<uint32_t>(y);

        for (int x = 0; x < srcW; x++) {
            const int uv_offset = (x >> 1) * uvPixelStride;
            dstRow[x] = YUV2RGB(pY[x], pU[uv_offset], pV[uv_offset]);
        }
    }
}
